...
{
    public class ValidatorProfileIntegrationTest : IClassFixture<WebApplicationFactory<Startup>>
    {
        public class HelloRequestValidatorProfile : ValidatorProfileBase
        {
            public HelloRequestValidatorProfile()
            {
                CreateInlineValidator<HelloRequest>()
                    .RuleFor(r => r.Name).NotEmpty();
            }
        }
        
        public ValidatorProfileIntegrationTest(WebApplicationFactory<Startup> factory)
        {
            _factory = factory
                .WithWebHostBuilder(builder => builder.ConfigureTestServices(services =>
                {
                    services.AddValidatorProfile<HelloRequestValidatorProfile>();
                }));
        }
